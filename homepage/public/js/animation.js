const textAnimation = (from, to, duration, ...timeOffsets) => {
    let titleWords = document.getElementById('titleWords').getElementsByTagName('span')
    ...
    const nextWord = (index) => {
        setTimeout(() => {
            ...
            nextWord(index + 1)
        }, index >= timeOffsets.length ? timeOffsets[index] + duration : timeOffsets[timeOffsets.length - 1] + duration)
    }
